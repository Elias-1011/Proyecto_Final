...
void Enemigo::razonar() {
    ...
    if (jugadorAtacando && puedeReaccionar) {
        ...
        if (r < m_probEsquivar) {
            m_estadoActual = EstadoAgente::ESQUIVAR; return;
        } else if (r < m_probEsquivar + m_probSaltar) {
            m_estadoActual = EstadoAgente::SALTAR;   return;
        }
    }
    ...
    if (jugadorEstaCerca()) {
        ...
        if (r < m_perfil.probRetrocederSiCerca) {
            m_estadoActual = EstadoAgente::RETROCEDER; return;
        }
    }
    ...
    case EstadoAgente::AVANZAR:
        if (m_distanciaAlJugador <= m_distanciaObjetivo) {
            if (puedeAtacar()) m_estadoActual = EstadoAgente::ATACAR;
            else {
                m_estadoActual = EstadoAgente::ESPERAR;
...
void Enemigo::actuar(float dt) {
    ...
    switch (m_estadoActual) {
    case EstadoAgente::ESPERAR:    m_tiempoTranscurrido += dt;                      break;
    case EstadoAgente::AVANZAR:    mover(dirAlJugador);                             break;
    case EstadoAgente::RETROCEDER: mover(-dirAlJugador);                            break;
    case EstadoAgente::SALTAR:     if (!m_enSuelo) break; saltar();                 break;
    case EstadoAgente::ESQUIVAR:   if (!m_esquivando) esquivar(-dirAlJugador);      break;
    case EstadoAgente::ATACAR:
        if (puedeAtacar() && m_distanciaAlJugador <= m_distanciaObjetivo + 20.0f)
            atacar(dirAlJugador);
        break;
    }
}
...
void Enemigo::aprender() {
    ...
    for (Accion a : m_historial) {
        if (a == Accion::ATACAR) {
            if (m_distanciaAlJugador < m_distanciaMinima * 1.5f) m_perfil.ataquesCortoRango++;
            else                                                   m_perfil.ataquesMedioRango++;
        } else if (a == Accion::ESQUIVAR) {
            if (jugador->getX() < m_x) m_perfil.esquivesIzquierda++;
            else                        m_perfil.esquivesDerecha++;
...
void Enemigo::ajustarDificultad(bool jugadorGanoRonda) {
    if (jugadorGanoRonda) {
        m_tiempoEsperaMin   = clamp(m_tiempoEsperaMin - 0.08f, ESPERA_MIN_TOPE, m_tiempoEsperaMax);
        m_tiempoEsperaMax   = clamp(m_tiempoEsperaMax - 0.15f, m_tiempoEsperaMin, ESPERA_MAX_TOPE);
        ...
        m_velEmbestida      = clamp(m_velEmbestida + 30.0f, VEL_EMBESTIDA_MIN, VEL_EMBESTIDA_MAX);
        m_distanciaObjetivo = clamp(m_distanciaObjetivo + 15.0f, DIST_OBJ_MIN, DIST_OBJ_MAX);
    } else {
        m_tiempoEsperaMin   = clamp(m_tiempoEsperaMin + 0.08f, ESPERA_MIN_TOPE, ESPERA_MAX_TOPE);
        m_tiempoEsperaMax   = clamp(m_tiempoEsperaMax + 0.15f, m_tiempoEsperaMin, ESPERA_MAX_TOPE);
        m_probEsquivar      = clamp(m_probEsquivar - 0.07f, PROB_MIN_TOPE, PROB_MAX_TOPE);
        m_probSaltar        = clamp(m_probSaltar   - 0.05f, PROB_MIN_TOPE, PROB_MAX_TOPE);
        m_velEmbestida      = clamp(m_velEmbestida - 30.0f, VEL_EMBESTIDA_MIN, VEL_EMBESTIDA_MAX);
...
float Enemigo::calcularDireccionAlJugador() const {
    Principal* jugador = m_referenciaEstado->getJugador();
    return (jugador->getX() - m_x >= 0.0f) ? 1.0f : -1.0f;
}
...
bool Enemigo::lanzarMoneda(float probabilidad) const {
    return (static_cast<float>(std::rand()) / RAND_MAX) < probabilidad;
}
